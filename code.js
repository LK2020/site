...
let firstOperand = '';
// текущий оператор- может быть последним или нет
let currentOperator = '';
...
//получаем кнопки и запускаем функцию
...
function processButton(value) {
    ...
    if (isNumber(value)) {
        ...
        // 1. когда была введена оперция и нужно второе число
        // 2. когда нажали равно
        // 3. ввод первого операнда для операции- в начале работы- операнд 0
        if (operandExpected) {
...
function calculate(firstOperand, secondOperandStr, opeartor) {
    ...
    switch (opeartor) {
        ...
        case '*':
            ...
            break
        case '/':
            // делить на ноль нельзя выдаем ошибку(стандартный калькулятор)
            if (num2 == 0) {
                return ['', 'err'];
            }
            total = num1 / num2;
